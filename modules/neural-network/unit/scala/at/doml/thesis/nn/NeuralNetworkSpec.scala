  ...
  "A NeuralNetwork" - {
    ...
    "should correctly calculate output for some input Vec" in {
      val inLayer = Layer[2, 2](
        vec[Neuron[2], 2](Neuron[2](vec[Double, 2](1.0, 2.0), 10.0), Neuron[2](vec[Double, 2](3.0, 4.0), 100.0))
      )
      val middleLayer = Layer[2, 3](
        vec[Neuron[2], 3](
          Neuron[2](vec[Double, 2](-1.0, -2.0), -10.0),
          Neuron[2](vec[Double, 2](-3.0, -4.0), -100.0),
          Neuron[2](vec[Double, 2](-5.0, -6.0), -1000.0)
        )
      )
      val outLayer = Layer[3, 2](
        vec[Neuron[3], 2](
          Neuron[3](vec[Double, 3](10.0, 20.0, 30.0), 1.0),
          Neuron[3](vec[Double, 3](40.0, 50.0, 60.0), 2.0)
        )
      )
